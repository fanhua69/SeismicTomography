...
/*		
		  ///////////////////////////////////////////////////////////////////////////////////////////////////
		  // Show the progress of the explode to CDC 
		int nMaxSlow=3000;
		int nLargerX=12;
		int nLargerY=12;	       
		for(int i=0;i<100;i++){
...
bool CTrace::MakeFlags()
{
	...
	for(i=0;i<m_sizeScan.cx;i++){
		for(j=0;j<m_sizeScan.cy;j++){
			...
			pPointRoute[0]=CMyPoint(0,0);
			pPointRoute[1]=CMyPoint(i,j);
			long nPoint=2;
			double m,n;
			if(i==0){
				for(n=1;n<=j;n++){
					pPointRoute[nPoint]=CMyPoint(0,n);
					nPoint++;
				}
			}
			else if(j==0){
				for(m=1;m<=i;m++){
					pPointRoute[nPoint]=CMyPoint(m,0);
					nPoint++;
				}
			}
			else{
...
CObArray& CTrace::GetRoute(CMyPoint &pointDest)
{
	...
	if(pointDest.x>=m_nGridXNumber||pointDest.y>=m_nGridYNumber){
